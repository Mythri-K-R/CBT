...
new #[Layout('layouts.institution')] class extends Component {
    ...
    public string $importExamType  = 'neet';
    public string $importSubjectId = '';
    ...
    public function updatedImportExamType(): void
    {
        $this->importSubjectId = '';
        $this->importChapterId = '';
    }

    public function updatedImportSubjectId(): void
    {
        $this->importChapterId = '';
    }
    ...
    public function importQuestions(): void
    {
        $this->validate([
            ...
            'importExamType' => 'required|in:neet,jee_main,jee_advanced,kcet',
        ]);

        $ext  = $this->importFile->getClientOriginalExtension();
        $path = $this->importFile->store('question-imports');
        ...
        $import = \App\Models\QuestionImport::create([
            ...
            'file_path'      => $path,
            'exam_type'      => $this->importExamType,
            'subject_id'     => $this->importSubjectId ?: null,
            'chapter_id'     => $this->importChapterId ?: null,
            'status'         => 'pending',
        ]);

        \App\Jobs\ProcessQuestionImport::dispatch($import->id);

        $this->showImportModal = false;
        $this->importFile      = null;
        $this->importSubjectId = '';
        $this->importChapterId = '';
        $this->importExamType  = 'neet';
        session()->flash('status', 'Import queued! Questions will appear shortly.');
    }
    ...
    public function with(): array
    {
        ...
        $importSubjects = Subject::withoutGlobalScopes()
            ->where('exam_type', $this->importExamType)
            ->where(function($q) { $q->whereNull('institution_id')->orWhere('institution_id', Auth::user()->institution_id); })
            ->orderBy('display_order')
            ->get(['id','name']);

        $importChapters = $this->importSubjectId
            ? Chapter::where('subject_id', $this->importSubjectId)->orderBy('display_order')->get(['id','name'])
            : collect();
...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
    <div class="w-full max-w-lg rounded-2xl bg-card border border-border shadow-2xl">
        <div class="flex items-center justify-between px-6 pt-6 pb-4 border-b border-border/50">
            <div>
                <h2 class="font-display text-xl font-bold">Bulk Import Questions</h2>
                <p class="text-xs text-muted-foreground mt-0.5">Upload CSV, Excel, or JSON — chapter-wise</p>
            </div>
        ...
        <form wire:submit="importQuestions" class="p-6 space-y-4">
            <div class="grid grid-cols-3 gap-3">
                <div>
                    <label class="block text-xs font-semibold mb-1.5 text-muted-foreground uppercase tracking-wide">Exam Type <span class="text-destructive">*</span></label>
                    <select wire:model.live="importExamType" class="h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-1 focus:ring-ring">
                ...
                <div>
                    <label class="block text-xs font-semibold mb-1.5 text-muted-foreground uppercase tracking-wide">Subject</label>
                    <select wire:model.live="importSubjectId" class="h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-1 focus:ring-ring">
                        <option value="">All Subjects</option>
                        @foreach($importSubjects as $sub)
                        <option value="{{ $sub->id }}">{{ $sub->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label class="block text-xs font-semibold mb-1.5 text-muted-foreground uppercase tracking-wide">Chapter</label>
                    <select wire:model="importChapterId" {{ $importChapters->isEmpty() ? 'disabled' : '' }} class="h-9 w-full rounded-md border border-input bg-background px-3 text-sm focus:outline-none focus:ring-1 focus:ring-ring disabled:opacity-50">
                        <option value="">All Chapters</option>
                        @foreach($importChapters as $ch)
            ...
            <div>
                <label class="block text-xs font-semibold mb-1.5 text-muted-foreground uppercase tracking-wide">File <span class="text-destructive">*</span></label>
                <label class="flex flex-col items-center justify-center gap-2 h-28 w-full rounded-lg border-2 border-dashed border-border bg-muted/20 hover:bg-muted/40 cursor-pointer transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" class="text-muted-foreground"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" x2="12" y1="3" y2="15"/></svg>
                    <span class="text-sm text-muted-foreground">
                        @if($importFile)
                            <span class="text-foreground font-medium">{{ $importFile->getClientOriginalName() }}</span>
                        @else
                            Click to select CSV / Excel / JSON
                        @endif
                    </span>
                    <span class="text-xs text-muted-foreground">Max 10 MB</span>
                    <input wire:model="importFile" type="file" accept=".csv,.xlsx,.xls,.json" class="hidden">
                </label>
                @error('importFile')<p class="text-xs text-destructive mt-1">{{ $message }}</p>@enderror
            </div>

            <div class="rounded-lg bg-muted/40 border border-border/50 p-3 text-xs text-muted-foreground space-y-1">
                <p class="font-semibold text-foreground">CSV columns required:</p>
                <p class="font-mono text-[10px] break-all">question_text, correct_answer, subject_id, chapter_id, exam_type, type, difficulty, option_A, option_B, option_C, option_D, positive_marks, negative_marks, explanation</p>
                <p class="mt-1">First row must be the header. Use <span class="font-medium text-foreground">subject_id</span> and <span class="font-medium text-foreground">chapter_id</span> numeric IDs from the system.</p>
            </div>
            ...
            <div class="flex items-center gap-3 pt-1">
                <button type="submit"
                    ...
                    <span wire:loading.remove wire:target="importQuestions">Import Questions</span>
                    <span wire:loading wire:target="importQuestions">Uploading...</span>
                </button>
